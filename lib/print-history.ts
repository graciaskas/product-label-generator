  ...
  static getStats() {
    ...
    return {
      ...
      today: history.filter(
        (entry) => new Date(entry.printedAt).toDateString() === today
      ).length,
      thisWeek: history.filter((entry) => new Date(entry.printedAt) >= thisWeek)
        .length,
      thisMonth: history.filter(
        (entry) => new Date(entry.printedAt) >= thisMonth
      ).length,
